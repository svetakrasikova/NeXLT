...
var Sort = (function(){
	...
	sort.length = function(a,b) {
		
		//REmove Show button :-(
		var ai = a.replace(/...Show$/,"");
		var bi = b.replace(/...Show$/,"");
		return sort.numeric(ai.length)-sort.numeric(bi.length);
		//return sort.numeric(a.length)-sort.numeric(b.length);
	};
	sort.inv_length = function(a,b) {
		return sort.length(b,a);
	};
...
//alert("Filter debug: "+strSel);
            if (strSel.substr(0,6) == "/(^$)|")
               strSel = "/.*$";
            return strSel.slice(0,strSel.length-1)+"/";
}
/**
	...
	*/
	table.filter = function(o,filters,args) {
		...
		if (!filters) {
			...
			// Allow for passing a select list in as the filter, since this is common design
			if (filters.nodeName=="SELECT" && filters.type=="select-multiple" && filters.selectedIndex>-1) { 
				if(filters.options[filters.selectedIndex].value.match(/^function/)) {
					filters={ 'filter':filters.options[filters.selectedIndex].value };
				}
				else
				filters={ 'filter':outputSelected(filters.options) };
			}
	...
	*/
	table.scrape = function(o,args) {
		...
		if (tdata.container_filtered_count) {
			tdata.container_filtered_count.innerHTML = "Count after filtering: " + unfilteredrowcount;
		}
			...
			if (hasClass(cell,table.FilterableClassName)) {
				...
				if (colValues.length>0) {
					if (typeof(args.insert)=="function") {
						...
						var sel = '<select multiple="multiple" size="4" onchange="Table.filter(this,this);" onclick="Table.cancelBubble(event)" class="'+table.AutoFilterClassName+'"><option value="">'+table.FilterAllLabel+'</option>';
						for (var i=0; i<colValues.length; i++) {
							sel += '<option value="'+colValues[i]+'">'+colValues[i]+'</option>';
						}
						sel += '</select>';
						cell.innerHTML = sel + cell.innerHTML;
